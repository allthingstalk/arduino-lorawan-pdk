 ...
 *  -> http://www.allthingstalk.com/lorawan-rapid-development-kit
 */
...
bool trySendGPSFix()
{
  ...
  if (sodaq_gps.tryScan(10))
  {
    ...
    debugSerial.println(sizeof(gpsData));

    container.addToQueue(float(sodaq_gps.getLat()), float(sodaq_gps.getLon()), float(sodaq_gps.getAlt()), 0., GPS, false);  // without ACK
    ...
    container.addToQueue((float)sodaq_gps.getSpeed(), NUMBER_SENSOR, false);  // without ACK
...
{   
  initPower();
  sodaq_gps.init();  // do this as early as possible so we have a working gps
